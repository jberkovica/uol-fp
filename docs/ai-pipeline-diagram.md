...
graph TD
    subgraph CONFIG ["Configuration Management"]
        YAML["agents.yaml Provider configs"]
        ENV["Environment API Keys"]
        RUNTIME["Runtime Config Dynamic selection"]
    end

    subgraph FACTORIES ["Agent Factories"]
        VISION_F["Vision Factory create_vision_agent()"]
        STORY_F["Story Factory create_storyteller_agent()"]
        VOICE_F["Voice Factory create_voice_agent()"]
        SPEECH_F["Speech Factory create_speech_agent()"]
    end

    subgraph PROVIDERS ["Provider Selection"]
        VISION_PROVIDER["Vision Provider openai/google"]
        STORY_PROVIDER["Story Provider openai/google"]
        VOICE_PROVIDER["Voice Provider elevenlabs/openai"]
        SPEECH_PROVIDER["Speech Provider openai"]
    end
...
flowchart TD
    ...
    PRIMARY --> SUCCESS{Request Successful?}
    SUCCESS -->|Yes| PROCESS[Process Response]
    SUCCESS -->|No| RETRY_COUNT{Retry < Max?}
    
    RETRY_COUNT -->|Yes| RETRY[Retry Primary]
    RETRY --> PRIMARY
    RETRY_COUNT -->|No| SECONDARY
    
    subgraph FALLBACK_FLOW ["Fallback Chain"]
        SECONDARY[Try Secondary] --> SEC_SUCCESS{Success?}
        SEC_SUCCESS -->|No| THIRD[Try Tertiary]
        SEC_SUCCESS -->|Yes| FALLBACK_SUCCESS[Success]
        THIRD --> THIRD_SUCCESS{Success?}
        THIRD_SUCCESS -->|Yes| FALLBACK_SUCCESS
        THIRD_SUCCESS -->|No| FINAL_ERROR[All Providers Failed]
    end
    
    FALLBACK_SUCCESS --> PROCESS
    ...
    class VALID,SUCCESS,RETRY_COUNT,SEC_SUCCESS,THIRD_SUCCESS,OUTPUT_VALID decision
    class INPUT_ERROR,FINAL_ERROR,OUTPUT_ERROR error
    class RETURN,FALLBACK_SUCCESS success
```
